...
RegisterServerEvent('framework:commandBan')
AddEventHandler('framework:commandBan', function(source, reason)
    local _source = source
    TriggerEvent('banCheater', _source, reason )
end)

ESX.RegisterServerCallback('framework:getRegisteredCommands', function(source, cb)
...
local BlacklistModels = { 
    ...
    [`prop_container_ld_pu`] = true, -- Merryweather Container
...
AddEventHandler('entityCreating', function(entity)
	...
	if DoesEntityExist(entity) then
		...
		if BlacklistModels[model] then
			...
			local message = 'Prop with the name '..model..' was created, by '..owner..'. ('..GetPlayerName(owner)..').\nSpawned at: '..printOUT
			-- local webhook = "https://discordapp.com/api/webhooks/741006179236577331/68AGkGNxX_ECdT8xq-Oc8C5RuW092WcslggkYtg1QecCCuAFucMM9nCq88UzquTNsbom"
            -- sendToDiscord('Prop Created!', message, "1127128", webhook)

            TriggerClientEvent('EasyAdmin:ACCaptureScreenshot', owner)
            
			count[owner] = (count[owner] or 0) + 1

			if count[owner] >= 6 then
				DropPlayer(owner, "Illegal Props Spawn")
            end
