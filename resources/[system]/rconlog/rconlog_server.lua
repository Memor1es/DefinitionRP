...
AddEventHandler('rconCommand', function(commandName, args)
    ...
    elseif commandName:lower() == 'tempbanclient' then
        ...
        CancelEvent()
    elseif commandName:lower() == 'charinfo' then

        _source = tonumber(args[1])
        
        xPlayer = ESX.GetPlayerFromId(_source)

        playerElement = {}
        playerElement.name = GetPlayerName(_source)
        playerElement.cash = xPlayer.getMoney()
        playerElement.bank = xPlayer.getAccount('bank').money
        playerElement.dirty = xPlayer.getAccount('black_money').money


        RconPrint(json.encode(playerElement))
        CancelEvent()
    end
end)
