function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition)
	...
	self.setMoney = function(money)
		...
		if money >= 0 then
			...
			TriggerEvent('discordbot:moneylog', self.source, " ```diff\n+ $"..money.."\n Money Set Using Command (?)```")
		else
	...
	self.addMoney = function(money)
		...
		if money >= 0 then
			if money >= 50000 then
				TriggerEvent('discordbot:moneylog', self.source, " <@&672205395229409291> <@&620730564949049344> <@&637431761407705109>\nLarge Money Movement Warning")
			end
			self.player.addMoney(money)
			local balance = self.player.get('money')
