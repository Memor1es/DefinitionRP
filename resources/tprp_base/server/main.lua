...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = "inform", text = "You have gave "..itemCount.."x of "..ESX.Items[itemName].label.." to "..targetXPlayer.name, length = 5000})
				TriggerClientEvent('mythic_notify:client:SendAlert', target, {type = "inform", text = "You have received "..itemCount.."x from "..ESX.Items[itemName].label.." to "..sourceXPlayer.name, length = 5000})
			end
	...
	elseif type == 'item_money' then
		...
		if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = "inform", text = "You have gave "..ESX.Math.GroupDigits(itemCount).." to "..targetXPlayer.name, length = 5000})
			TriggerClientEvent('mythic_notify:client:SendAlert', target, {type = "inform", text = "You have received "..ESX.Math.GroupDigits(itemCount).." from "..sourceXPlayer.name, length = 5000})
	...
	elseif type == 'item_account' then
		...
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = "inform", text = "You have gave "..ESX.Math.GroupDigits(itemCount).." to "..targetXPlayer.name.." in thier "..Config.AccountLabels[itemName], length = 5000})
			TriggerClientEvent('mythic_notify:client:SendAlert', target, {type = "inform", text = "You have received "..ESX.Math.GroupDigits(itemCount).." from "..sourceXPlayer.name.." into your "..Config.AccountLabels[itemName], length = 5000})
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = "error", text = "Invalid Amount", length = 5000})
		end
...
AddEventHandler('esx:removeInventoryItem', function(type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount == nil or itemCount < 1 then
			TriggerClientEvent('mythic_notify:client:SendAlert', _sourceource, {type = "error", text = "Invalid Amount", length = 5000})
		else
			local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('esx:useItem', function(itemName)
	...
	if count > 0 then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, {type = "error", text = "Action is not possible.", length = 5000})
	end
end)
...
AddEventHandler('esx:onPickup', function(id)
	...
	if pickup.type == 'item_standard' then
		...
		if remaining > 0 then
			TriggerClientEvent('mythic_notify:client:SendAlert', _source, {type = "error", text = "Can't pickup "..item.label.." due to insufficent inventory space", length = 5000})
