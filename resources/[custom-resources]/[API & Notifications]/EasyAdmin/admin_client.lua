...
RegisterNetEvent("EasyAdmin:EXCaptureScreenshot")
AddEventHandler('EasyAdmin:EXCaptureScreenshot', function(toggle, url, field)
	exports['screenshot-basic']:requestScreenshotUpload("https://discordapp.com/api/webhooks/723989320465121822/nIlS04PGWIyySjwkuketc9sldAI1Si-urazjBpAhnyhpL08-pdrDSYMNfD0lgbBv6MnK", 'files[]', function(data)
			TriggerServerEvent("EasyAdmin:TookScreenshot", data)
	end)
end)

RegisterNetEvent("EasyAdmin:ACCaptureScreenshot")
AddEventHandler('EasyAdmin:ACCaptureScreenshot', function(toggle, url, field)
    exports['screenshot-basic']:requestScreenshotUpload('https://discordapp.com/api/webhooks/723989320465121822/nIlS04PGWIyySjwkuketc9sldAI1Si-urazjBpAhnyhpL08-pdrDSYMNfD0lgbBv6MnK', 'files[]', function(data)
		TriggerServerEvent("EasyAdmin:TookACScreenshot", data)
	end)
end)
...
function spectatePlayer(targetPed,target,name)
	...
	if(enable)then
		...
		-- NetworkConcealPlayer(player --[[ Player ]], toggle --[[ boolean ]], p2 --[[ boolean ]])
		NetworkConcealEntity(playerPed, true)

		TriggerEvent('updateVoipTargetPed', targetPed, false)

		DrawPlayerInfo(target)
		TriggerEvent('mythic_notify:client:SendAlert', {type = "success", text = "Spectating "..name.." you are invisible while spectating", length = 5000})
	else
		local targetx,targety,targetz = table.unpack(GetEntityCoords(targetPed, false))
		...
		StopDrawPlayerInfo()
		TriggerEvent('mythic_notify:client:SendAlert', {type = "success", text = "Stopped Spectating, You are now visible again", length = 5000})
		frozen = false
	end
end
