...
RegisterNetEvent('EasyAdmin:Toko:Client:ReceiveRadioChannels')
AddEventHandler('EasyAdmin:Toko:Client:ReceiveRadioChannels', function(target, channels)
...
function DrawPlayerInfo(target)
	TriggerServerEvent('EasyAdmin:Toko:Server:RequestRadioChannel', GetPlayerServerId(target))
end
	...
	while true do
		...
		if drawInfo then
			...
			if radioChannels ~= nil then
				table.insert(text, "Radio Channels ")
				for k, v in pairs(radioChannels) do
					table.insert(text, "Channel ~g~"..k.."~w~: "..v)
				end
			else
				table.insert(text, "Radio Channel: ~r~Not on Radio~w~")
			end
			...
			for i,theText in pairs(text) do
				...
				EndTextCommandDisplayText(0.4, 0.7+(i/30))
			end
			...
			if IsControlJustPressed(0,103) then
				...
				NetworkConcealEntity(targetPed, false)
