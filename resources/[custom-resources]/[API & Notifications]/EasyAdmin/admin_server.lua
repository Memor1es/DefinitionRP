...
RegisterServerEvent('EasyAdmin:Toko:Server:RequestRadioChannel')
AddEventHandler('EasyAdmin:Toko:Server:RequestRadioChannel', function(target)
	local _src = source
	if exports['tokovoip_script'] then
		TriggerClientEvent('EasyAdmin:Toko:Client:ReceiveRadioChannels', _src, tonumber(target), exports['tokovoip_script']:getRadioChannels(tonumber(target)))
	else
		eeTriggerClientEvent('EasyAdmin:Toko:Client:ReceiveRadioChannels', _src, tonumber(target), nil)
	end
end)
...
Citizen.CreateThread(function()
	...
	AddEventHandler('EasyAdmin:amiadmin', function()
		...
		TriggerClientEvent('chat:addSuggestion', source, '/calladmin', "Call an admin", { {name='reason', help="enter text to send to admins"} })
		
		TriggerClientEvent('chat:addSuggestion', source, '/report', "Report a player", { {name='player id', help="the player's server id"}, {name='reason', help="the reason for report"}})
		...
		if GetConvar("ea_enableCallAdminCommand", "false") == "true" then
			...
			for i,_ in pairs(OnlineAdmins) do 
				...
				TriggerClientEvent('chat:addMessage', i, {
					template = '<div class="chat-message-system"><b>EasyAdmin -  {0} Called for an Admin:</b> {1}</div>',
					args = { user, msg }
				})
			end
			...
			TriggerClientEvent('chat:addMessage', source, {
				template = '<div class="chat-message-system"><b>Admin call sent.</b></div>',
			})
		end
	end, false)
	PlayerReports = {}
			...
			if id and valid then
				...
				if addReport then
					...
					for i,_ in pairs(OnlineAdmins) do 
						...
						local user = getName(source)
						local msg = "Report "..#PlayerReports[id].."/"..minimumreports.." Offender: ["..GetPlayerName(id).."] ID: ["..id.."] Reason: "..reason.."."

						TriggerClientEvent('chat:addMessage', i, {
							template = '<div class="chat-message-system"><b>EasyAdmin -  {0} filed a report:</b></br> {1}</div>',
							args = { user, msg }
						})
					end
					TriggerClientEvent('chat:addMessage', source, {
						template = '<div class="chat-message-system"><b>EasyAdmin - Report filed.</b></div>',
					})
	...
	AddEventHandler('EasyAdmin:TakeScreenshot', function(playerId)
		...
		if DoesPlayerHavePermission(source,"easyadmin.screenshot") then
			thistemporaryevent = AddEventHandler("EasyAdmin:TookScreenshot", function(result)
				res = tostring(result)
				if (moderationNotification == GetConvar("ea_screenshoturl", 'https://images.drp.wtf/up.php')) then
					res = ""
				end
				local webhook = "https://discordapp.com/api/webhooks/760922916224761856/x6_91Xle7H45MATpkELhruYoJkWfnBqWJswtAApZIQLadg9mW8JJnrxr3rbLkssHnW07"

				SendWebhookMessage(webhook, getName(src).. " took a screenshot of "..getName(playerId), getName(src), getName(playerId), res)
				...
				TriggerClientEvent("chat:addMessage", src, { template = 'Screenshot Uploaded' })
	...
	RegisterServerEvent('EasyAdmin:TookEXScreenshot')
	AddEventHandler("EasyAdmin:TookEXScreenshot", function(result)
		local src = source
		local name = GetPlayerName(src)
		res = tostring(result)
		TriggerEvent('discordbot:exlog', res)
	end)

	RegisterServerEvent('EasyAdmin:TookACScreenshot')
	AddEventHandler("EasyAdmin:TookACScreenshot", function(result)
		local src = source
		local name = GetPlayerName(src)
		local identifier = GetPlayerIdentifiers(src)[1]
		res = json.decode(result)
		-- res = tostring(result)
		-- SendWebhookMessage(moderationNotification, string.format(GetLocalisedText("admintookscreenshot"), res))
		local msg = "```css\n[ILLEGAL OBJECTS] ["..name.." | ID: "..src.." | Identifier: "..identifier.."] a screenshot has been taken due to illegal objects```"
		Wait(500)
		TriggerEvent('discordbot:exlog', msg)
