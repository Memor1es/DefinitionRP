...
function UpdatePlayerTable(connectedPlayers)
	...
	for k,v in pairs(connectedPlayers) do
		...
		if num == 1 then
			-- table.insert(formattedPlayerList, ('<tr><td><span style="color:#d1d1d1">%s</span></td><td>%s</td>'):format(v.id, v.name))
			table.insert(formattedPlayerList, ('<tr><td><span style="color:#d1d1d1">%s</span></td><td>%s</td>'):format(v.id, v.name))
			num = 1
		elseif num == 2 then
			table.insert(formattedPlayerList, ('<td><span style="color:#d1d1d1">%s</span></td><td>%s</td>'):format(v.id, v.name))
			num = 3
        elseif num == 3 then
            table.insert(formattedPlayerList, ('<td><span style="color:#d1d1d1">%s</span></td><td>%s</td>'):format(v.id, v.name))
            num = 1
...
--[[ Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
		if IsControlJustPressed(0, Keys['F10']) and IsInputDisabled(0) and not toggled then
			toggled = true
