      ...
      --  SetPedCanRagdoll(GetPlayerPed(-1), false)
        ...
        while IsDead do
            ...
            if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
                inveh = true
                TriggerEvent("deathAnim")
            elseif not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) and inveh and GetEntityHeightAboveGround(GetPlayerPed(-1)) < 2.0 or inveh == 0 and GetEntityHeightAboveGround(GetPlayerPed(-1)) < 2.0 then
                inveh = false
                TriggerEvent("deathAnim")
...
myanim = "dead_a"
vehanim = "pov_additive_steer_duck_centre"

local pedisincar = false
RegisterNetEvent('deathAnim')
AddEventHandler('deathAnim', function()
    local sleeptime = 3000
    if not dragged and not tryingAnim and not enteringveh and IsDead then
        tryingAnim = true
...
function SendDistressSignal()
    local text = "Call | Somebody is in need of assistance."
...
function SendDistressSignalEmergency()
    local text = "Call | Emergency Personell in need of assistance."
    local coords = GetEntityCoords(GetPlayerPed(-1))
    TriggerServerEvent('MF_Trackables:NotifyAll',text,coords,"ambulance",true,GetPlayerServerId(PlayerId()))
end

exports("SendDistressSignalEmergency", SendDistressSignalEmergency)
...
function releaseBody()
    ...
    ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailTime", function(inJail, newJailTime)
		if inJail then
            SetEntityCoords(GetPlayerPed(-1), 1772.30, 2552.00, 45.56)
            ...
            --[[ if isCop then
                ...
                SetEntityCoords(GetPlayerPed(-1), 327.386, -602.93, 43.2)
            -- end
...
AddEventHandler('heal', function()
    ...
    ResetPedMovementClipset(playerPed,1)
