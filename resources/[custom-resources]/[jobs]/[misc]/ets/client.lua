...
AddEventHandler('esx_ets:start', function(data, jobid)
    working = true
    local truck = loadVehicle(data.vehicles[1], data.start[1], data.start[2])
    local trailer = loadVehicle(data.vehicles[2], data.trailer[1], data.trailer[2])

    local destinationBlip, truckBlip, trailerBlip = addBlip(data.arrive, 38, 3, Strings['destination'])
    while true do
        print(trailer)
        print(data.trailer[1])
