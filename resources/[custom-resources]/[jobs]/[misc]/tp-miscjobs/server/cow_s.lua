...
AddEventHandler('smerfikcraft:skup22', function()
    ...
    if jabka >= 1 then
        ...
        --[[ local multiplier = math.random(5,10) ]]
        local singleRemoveAmount = ESX.Math.Round(jabka / 4)
        local totalAmount = jabka
        ...
        TriggerClientEvent('sprzedawanie:jablekanim22', _source)
        Citizen.Wait(4000)
        xPlayer.addMoney(math.ceil(totalAmount * cena))
        TriggerClientEvent('odlacz:propa2', _source)
        TriggerClientEvent('esx:showHelpNotification', _source, 'Milked cow of ~y~' .. ESX.Math.Round(totalAmount) ..' ~w~buckets: ~g~$' .. math.ceil(totalAmount * cena) .. '')
