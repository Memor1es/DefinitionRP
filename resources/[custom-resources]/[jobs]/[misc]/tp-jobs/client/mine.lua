...
X.Craft = function(item)
    ...
    Citizen.CreateThread(
        function()
            ...
            exports["t0sic_loadingbar"]:StartDelayedFunction(
                "Melting ores",
                10000,
                function()
    ...
    for i, v in pairs(Config.Rocks) do
        if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.x, v.y, v.z) < 75 then
            if v.health >= v.max / 2 then
                X.DrawText3D(
                    {x = v.x, y = v.y, z = v.z},
                    Local.rockOf .. "~b~" .. Local[v.data] .. "~w~ : ~g~" .. v.health .. "/" .. v.max
                )
            elseif v.health >= v.max / 4 then
                X.DrawText3D(
                    {x = v.x, y = v.y, z = v.z},
                    Local.rockOf .. "~b~" .. Local[v.data] .. "~w~ : ~b~" .. v.health .. "/" .. v.max
                )
            elseif v.health < v.max / 4 and v.health ~= 0 then
                X.DrawText3D(
                    {x = v.x, y = v.y, z = v.z},
                    Local.rockOf .. "~b~" .. Local[v.data] .. "~w~ : ~y~" .. v.health .. "/" .. v.max
                )
            elseif v.health <= 0 then
                X.DrawText3D(
                    {x = v.x, y = v.y, z = v.z},
                    Local.rockOf .. "~b~" .. Local[v.data] .. "~w~ : ~r~ " .. v.health .. "/" .. v.max
                )
            end
        end
