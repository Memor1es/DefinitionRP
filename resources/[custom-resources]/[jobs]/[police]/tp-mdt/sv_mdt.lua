...
local DiscordWebhookPoliceMDT = "https://discordapp.com/api/webhooks/748657114808844328/euhsdNYPTUOLpvVLuQS65WgwIDn5SZnVBiOMdBzByuGwDbWSrZmNu8YNMHz9yFoow8Js"
...
AddEventHandler("mdt:saveReportChanges", function(data)
	local src = source
	local author = GetCharacterName(src)
	local steamname = GetPlayerName(src)
	local steamid = ESX.GetPlayerFromId(src).getIdentifier()
	...
	TriggerEvent('discordbot:mdtlog', '```css\n'..author..' | ' ..steamname.. ' [Steam ID: ' ..steamid.. '] | has edited the details of report ['..data.title..' - ' ..data.id..'] at ['..data.date..']\n```')
end)
...
AddEventHandler("mdt:deleteReport", function(id)
	...
	TriggerEvent('discordbot:mdtlog', '```css\n'..author..' | '..steamname.. ' [Steam ID: ' ..steamid.. '] | has deleted report ['..id..'] at ['..date..']\n```')
end)
...
AddEventHandler("mdt:submitNewReport", function(data)
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`, `jailtime`) VALUES (@id, @title, @incident, @charges, @author, @name, @date, @sentence)', {
		...
		TriggerEvent('discordbot:mdtlog', '```css\n'..author..' | '.. GetPlayerName(usource) .. ' [Steam ID: ' .. ESX.GetPlayerFromId(usource).getIdentifier() .. '] | has created report ['..data.title..' - ' ..id..'] for '..data.name..' at ['..data.date..']\n```')
	end)
...
AddEventHandler("mdt:submitNewWarrant", function(data)
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		TriggerEvent('discordbot:mdtlog', '```css\n'..data.author..' | '.. GetPlayerName(usource) .. ' [Steam ID: ' .. ESX.GetPlayerFromId(usource).getIdentifier() .. '] | has created a warrant on report ['..data.report_title..' - ' ..data.report_id..'] for '..data.name..' at ['..data.date..']\n```')
	end)
end)
...
AddEventHandler("mdt:deleteWarrant", function(id)
	...
	MySQL.Async.execute('DELETE FROM `mdt_warrants` WHERE `id` = @id', {
		...
		TriggerClientEvent("mdt:completedWarrantAction", usource)
		TriggerEvent('discordbot:mdtlog', '```css\n'..author.. ' | ' .. GetPlayerName(usource) .. ' [Steam ID: ' .. ESX.GetPlayerFromId(usource).getIdentifier() .. '] | has deleted warrant [' .. id.. '] at ['..date..']\n```')
	end)
end)
