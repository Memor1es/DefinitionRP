...
TriggerEvent('es:addCommand', 'legalmdt', function(source, args, user)
    ...
    if xPlayer.job.name == 'lawyer' then
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		...
    		MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			TriggerClientEvent('h4rv3y-mdt-legal:toggleVisibilty', usource, reports, warrants, officer)
    		end)
    	end)
    end
end)

RegisterServerEvent("h4rv3y-mdt-legal:hotKeyOpen")
AddEventHandler("h4rv3y-mdt-legal:hotKeyOpen", function()
	local usource = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == 'lawyer' then
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
...
RegisterServerEvent("h4rv3y-mdt-legal:getOffensesAndOfficer")
AddEventHandler("h4rv3y-mdt-legal:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	local jailtime = {}
	MySQL.Async.fetchAll('SELECT * FROM fine_types', {
		...
		TriggerClientEvent("h4rv3y-mdt-legal:returnOffensesAndOfficer", usource, charges, officer)
	end)
end)

RegisterServerEvent("h4rv3y-mdt-legal:performOffenderSearch")
AddEventHandler("h4rv3y-mdt-legal:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE LOWER(`firstname`) LIKE @query OR LOWER(`lastname`) LIKE @query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE @query", {
		...
		TriggerClientEvent("h4rv3y-mdt-legal:returnOffenderSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("h4rv3y-mdt-legal:getOffenderDetails")
AddEventHandler("h4rv3y-mdt-legal:getOffenderDetails", function(offender)
	local usource = source
	GetLicenses(offender.identifier, function(licenses) offender.licenses = licenses end)
	while offender.licenses == nil do Wait(0) end
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		MySQL.Async.fetchAll('SELECT * FROM `user_convictions` WHERE `char_id` = @id', {
			...
			MySQL.Async.fetchAll('SELECT * FROM `mdt_warrants` WHERE `char_id` = @id', {
				...
				TriggerClientEvent("h4rv3y-mdt-legal:returnOffenderDetails", usource, offender)
			end)
		end)
	end)
end)

RegisterServerEvent("h4rv3y-mdt-legal:getOffenderDetailsById")
AddEventHandler("h4rv3y-mdt-legal:getOffenderDetailsById", function(char_id)
	local usource = source
...
RegisterServerEvent("h4rv3y-mdt-legal:saveOffenderChanges")
AddEventHandler("h4rv3y-mdt-legal:saveOffenderChanges", function(id, changes, identifier)
...
RegisterServerEvent("h4rv3y-mdt-legal:saveReportChanges")
AddEventHandler("h4rv3y-mdt-legal:saveReportChanges", function(data)
...
RegisterServerEvent("h4rv3y-mdt-legal:deleteReport")
AddEventHandler("h4rv3y-mdt-legal:deleteReport", function(id)
	MySQL.Async.execute('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		['@id']  = id
	})
end)

RegisterServerEvent("h4rv3y-mdt-legal:submitNewReport")
AddEventHandler("h4rv3y-mdt-legal:submitNewReport", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`, `jailtime`) VALUES (@id, @title, @incident, @charges, @author, @name, @date, @sentence)', {
		...
		TriggerEvent("h4rv3y-mdt-legal:getReportDetailsById", id, usource)
	end)
...
RegisterServerEvent("h4rv3y-mdt-legal:sentencePlayer")
AddEventHandler("h4rv3y-mdt-legal:sentencePlayer", function(jailtime, charges, char_id, fine, players)
	local usource = source
	local jailmsg = ""
	local xPlayer = ESX.GetPlayerFromId(source)
			...
			MySQL.Async.fetchAll('SELECT * FROM `characters` WHERE `identifier` = @identifier', {
				...
				if result[1].id == char_id then
					if jailtime and jailtime > 0 then
						...
						TriggerEvent("nw-jail:jailPlayer", src, jailtime, jailmsg)
					end
					return
				end
			end)
...
RegisterServerEvent("h4rv3y-mdt-legal:performReportSearch")
AddEventHandler("h4rv3y-mdt-legal:performReportSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		...
		TriggerClientEvent("h4rv3y-mdt-legal:returnReportSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("h4rv3y-mdt-legal:performVehicleSearch")
AddEventHandler("h4rv3y-mdt-legal:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE LOWER(`plate`) LIKE @query", {
		...
		TriggerClientEvent("h4rv3y-mdt-legal:returnVehicleSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("h4rv3y-mdt-legal:getVehicle")
AddEventHandler("h4rv3y-mdt-legal:getVehicle", function(vehicle)
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE `identifier` = @query", {
		...
		TriggerClientEvent("h4rv3y-mdt-legal:returnVehicleDetails", usource, vehicle)
	end)
end)

RegisterServerEvent("h4rv3y-mdt-legal:getWarrants")
AddEventHandler("h4rv3y-mdt-legal:getWarrants", function()
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		...
		TriggerClientEvent("h4rv3y-mdt-legal:returnWarrants", usource, warrants)
	end)
end)

RegisterServerEvent("h4rv3y-mdt-legal:submitNewWarrant")
AddEventHandler("h4rv3y-mdt-legal:submitNewWarrant", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		TriggerClientEvent("h4rv3y-mdt-legal:completedWarrantAction", usource)
	end)
end)

RegisterServerEvent("h4rv3y-mdt-legal:deleteWarrant")
AddEventHandler("h4rv3y-mdt-legal:deleteWarrant", function(id)
	local usource = source
...
RegisterServerEvent("h4rv3y-mdt-legal:getReportDetailsById")
AddEventHandler("h4rv3y-mdt-legal:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		...
		if result and result[1] then
			...
			TriggerClientEvent("h4rv3y-mdt-legal:returnReportDetails", usource, result[1])
		end
	end)
end)
