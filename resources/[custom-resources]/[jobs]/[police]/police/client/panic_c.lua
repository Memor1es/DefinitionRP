...
AddEventHandler("Pass-Alarm:Return:NewPanic", function(source, Officer)
	...
	if xPlayer.job.name == "police" or xPlayer.job.name == "ambulance" then
		...
		ESX.TriggerServerCallback('citizenid:getOtherPlayerData', function(data)
			TriggerServerEvent("tp:addChatPolice", "[URGENT] [10-13A] ".. data.firstname .." " .. data.lastname .." Is down on  " .. Officer.Location, 45000)
		end, GetPlayerServerId(Officer.Player))
...
RegisterNetEvent("Pass-Alarm:Return:NewPanic2")
AddEventHandler("Pass-Alarm:Return:NewPanic2", function(source, Officer)
	--if Panic.Tuned then
	local _source = source
	local xPlayer = ESX.GetPlayerData()
	local playername = xPlayer.name
	if xPlayer.job.name == "police" or xPlayer.job.name == "ambulance" then
		...
		-- Only people tuned to the panic channel can see the message
		ESX.TriggerServerCallback('citizenid:getOtherPlayerData', function(data)
			TriggerServerEvent("tp:addChatPolice2", "[10-13B] ".. data.firstname .." " .. data.lastname .." Is in need of assistance on  " .. Officer.Location, 45000)
		end, GetPlayerServerId(Officer.Player))

		local officerPed = GetPlayerPed(GetPlayerFromServerId(Officer.Player))
...
AddEventHandler("Pass-Alarm:Return:MedicNewPanic", function(source, Officer)
	...
	if xPlayer.job.name == "police" or xPlayer.job.name == "ambulance" then
		...
		-- Only people tuned to the panic channel can see the message
		ESX.TriggerServerCallback('citizenid:getOtherPlayerData', function(data)
			TriggerServerEvent("tp:addChatPolice", "[URGENT] [10-14A] ".. data.firstname .." " .. data.lastname .." Is in need of assistance on  " .. Officer.Location, 45000)
		end, GetPlayerServerId(Officer.Player))
...
RegisterNetEvent("Pass-Alarm:Return:MedicNewPanic2")
AddEventHandler("Pass-Alarm:Return:MedicNewPanic2", function(source, Officer)
	local _source = source
	local xPlayer =ESX.GetPlayerData()
	local playername = xPlayer.name
	...
	if xPlayer.job.name == "police" or xPlayer.job.name == "ambulance" then
		...
		-- Only people tuned to the panic channel can see the message
		ESX.TriggerServerCallback('citizenid:getOtherPlayerData', function(data)
			TriggerServerEvent("tp:addChatPolice2", "[10-14B] ".. data.firstname .." " .. data.lastname .." Is down on  " .. Officer.Location, 45000)
		end, GetPlayerServerId(Officer.Player))

		local officerPed = GetPlayerPed(GetPlayerFromServerId(Officer.Player))

		Citizen.CreateThread(function() -- 280
			local Blip = AddBlipForRadius(Officer.Coords.x, Officer.Coords.y, Officer.Coords.z, 100.0)
			SetBlipColour(Blip, 1)
			SetBlipAlpha(Blip, 60)
			SetBlipFlashes(Blip, true)
...
AddEventHandler("tp:panicTrigger", function()
	...
	if xPlayer.job.name == "police" then
        if Panic.Cooling == 0 then
            ...
            TriggerServerEvent("Police-Panic:NewPanic", Officer)
			exports['ambulancejob']:SendDistressSignalEmergency()
			Panic.Cooling = Config.Cooldown
        else
            exports['mythic_notify']:SendAlert('error', 'The Panic Button is still on a cooldown.')
        end
    else
        exports['mythic_notify']:SendAlert('error', 'You are not whitelisted for this command.')
    end
end)

RegisterNetEvent("tp:panicTrigger2")
AddEventHandler("tp:panicTrigger2", function()
    local _source = source
	local xPlayer = ESX.GetPlayerData()
	local playername = xPlayer.name
	...
	if xPlayer.job.name == "police" then
        if Panic.Cooling == 0 then
            ...
            TriggerServerEvent("Police-Panic:NewPanic2", Officer)

            Panic.Cooling = Config.CooldownB
        else
            exports['mythic_notify']:SendAlert('error', 'The Panic Button is still on a cooldown.')
        end
    else
...
RegisterNetEvent("tp:panicTriggerMedic2")
AddEventHandler("tp:panicTriggerMedic2", function()
    local _source = source
	local xPlayer =ESX.GetPlayerData()
	local playername = xPlayer.name
	...
	if xPlayer.job.name == "ambulance" then
        if Panic.Cooling == 0 then
            ...
            TriggerServerEvent("Medic-Panic:NewPanic2", Officer)
			exports['ambulancejob']:SendDistressSignalEmergency()
			Panic.Cooling = Config.CooldownB
        else
            exports['mythic_notify']:SendAlert('error', 'The Panic Button is still on a cooldown.')
