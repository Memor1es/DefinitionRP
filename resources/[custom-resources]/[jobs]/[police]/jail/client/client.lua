...
function JailLogin()
	...
	TriggerEvent('mythic_notify:client:SendAlert', {type = "inform", text = "Last time you went to sleep you we're in prison, we have returned you now.", length = 5000})
...
function UnJail()
	...
	TriggerEvent('mythic_notify:client:SendAlert', {type = "inform", text = "You have been released from prison.", length = 5000})
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			TriggerEvent('mythic_notify:client:SendAlert', {type = "inform", text = "You have "..jailTime.." "..month.." remaining on your sentence.", length = 10000})
			Citizen.Wait(60000)
			jailTime = jailTime - 1
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								TriggerEvent('mythic_notify:client:SendAlert', {type = "error", text = "You have already taken this package.", length = 5000})
							end
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
			...
			TriggerEvent('mythic_notify:client:SendAlert', {type = "error", text = "Cancelled.", length = 5000})
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
				...
				if jailTime == nil then
					TriggerEvent('mythic_notify:client:SendAlert', {type = "error", text = "The time has to be in months", length = 5000})
            	else
              		menu2.close()
					  ...
					  if closestPlayer == -1 or closestDistance > 3.0 then
						TriggerEvent('mythic_notify:client:SendAlert', {type = "error", text = "No Players Nearby", length = 5000})
