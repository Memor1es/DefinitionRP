RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job.name == "police" then
		...
		TriggerEvent('mythic_notify:client:SendAlert', {type = "error", text = "You are not a Law Emforcement Officer", length = 5000})
	end
end)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Security" or action == "Jail" then
				if jailTime >= 1 then
						TriggerEvent('mythic_notify:client:SendAlert', {type = "error", text = "You are in jail", length = 5000})
						return
				end
				if PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" then
					TriggerEvent('mythic_notify:client:SendAlert', {type = "success", text = "You used your government keycard", length = 5000})
				else
					TriggerEvent('mythic_notify:client:SendAlert', {type = "error", text = "You do not have a key to go here", length = 5000})
					return
				end
			end
