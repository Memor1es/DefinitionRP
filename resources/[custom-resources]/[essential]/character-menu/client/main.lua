...
--[[ MAIN LOOP WHICH STARTS THE CHARACTER MENU ]]
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
...
AddEventHandler('kashactersC:SpawnCharacter', function(spawn, isnew, fullname)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if charSpawning1 then
            for i = 1, 125, 1 do
                DrawCharName(ChosenCharName, math.floor(255))
                DrawScreenText("Your Story Continues...", math.floor(255))
                Citizen.Wait(0)
            end
            charSpawning1 = false
            DrawCharName(ChosenCharName, math.floor(255))
            DrawScreenText("Your Story Continues...", math.floor(255))
            charSpawning2 = true
        elseif charSpawning2 then
            for i = 1000, 0, -1 do
                DrawCharName(ChosenCharName, math.floor((i/4)))
                DrawScreenText("Your Story Continues...", math.floor((i/4)))
                Citizen.Wait(0)
            end
            charSpawning2 = false
        end
...
function DrawScreenText(text, alpha)
    SetTextFont(4)
    ...
    DrawRect(0.5, 0.5, 1.0, 1.0, 0, 0, 0, alpha)
end
function DrawCharName(text, alpha)
    SetTextFont(1)
    SetTextProportional(1)
    SetTextColour(255, 255, 255, alpha)
    SetTextEdge(2, 0, 0, 0, 255)
