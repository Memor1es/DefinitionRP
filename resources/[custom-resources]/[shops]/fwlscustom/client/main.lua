...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	--ESX.TriggerServerCallback('esx_fwlscustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
end)
...
RegisterNetEvent('esx_fwlscustom:installMod')
AddEventHandler('esx_fwlscustom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_fwlscustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_fwlscustom:cancelInstallMod')
AddEventHandler('esx_fwlscustom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_fwlscustom:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('esx_fwlscustom:buyMod', price)
		...
		menu.close()
		TriggerEvent('esx_fwlscustom:cancelInstallMod')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'flywheels') or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
