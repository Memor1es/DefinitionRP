...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	--ESX.TriggerServerCallback('esx_relscustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
end)
...
RegisterNetEvent('esx_relscustom:installMod')
AddEventHandler('esx_relscustom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_relscustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_relscustom:cancelInstallMod')
AddEventHandler('esx_relscustom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_relscustom:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('esx_relscustom:buyMod', price)
		...
		menu.close()
		TriggerEvent('esx_relscustom:cancelInstallMod')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'racersedge') or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
