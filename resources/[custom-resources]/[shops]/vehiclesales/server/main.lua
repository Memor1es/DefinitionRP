...
ESX.RegisterServerCallback("esx_lg_vehiclesale:isVehicleValid", function(source, cb, vehicleProps, price, cardetails)
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
    local plate = vehicleProps["plate"]
	local isFound = 'no'
	local maxPrice = 0
	if cardetails ~= nil then
		MySQL.Async.fetchAll("SELECT * FROM `vehicles` WHERE `model` = @name", {['@name'] = cardetails.displayText}, function(dbvehicle)
			if dbvehicle[1] ~= nil then
				local price2 = dbvehicle[1].price
				local withPercent = (dbvehicle[1].price / 100 * 1)
				local addedPercent = math.floor((withPercent + price2))
				local workOut = (addedPercent / 2)
				local maxPrice = math.floor((price2 + workOut))
				...
				print('[^1ERROR^7] - Player tried selling a non existent database vehicle in the used car sales lot.')
			end
		end)
	end
end)
