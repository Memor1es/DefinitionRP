...
function takeHostage()
	...
	if not canTakeHostage then 
		exports['mythic_notify']:SendAlert('error', 'You need a pistol with ammo to take a hostage at gunpoint!', 7000)
	end

	if not IsEntityPlayingAnim(GetPlayerPed(-1), 'dead', 'dead_a', 3) then
		if not holdingHostageInProgress and canTakeHostage then		
			...
			if closestPlayer ~= nil then
				if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'random@mugging3', 'handsup_standing_base', 3) then
					...
					exports['mythic_notify']:SendAlert('error', 'The hostage needs their hands in the air to do this!', 7000)
				end
			else
				--print("[CMG Anim] No player nearby")
				exports['mythic_notify']:SendAlert('error', 'Nobody nearby to take hostage!', 7000)
			end 
		end
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DisableControlAction(0,23,true) -- disable getting in vehicle
