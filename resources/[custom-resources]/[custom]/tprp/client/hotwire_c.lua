...
RegisterNetEvent('lockpickingsomething')
AddEventHandler('lockpickingsomething', function()
  GetNearestPorV()
end)

function GetNearestPorV()
  local closestPlayer, closestPdistance = ESX.Game.GetClosestPlayer()
  local closestVehicle, closestVdistance = ESX.Game.GetClosestVehicle()

  if (closestPdistance < closestVdistance) and (closestPlayer ~= -1) then
      if closestPdistance > 3.0 then
          exports['mythic_notify']:DoCustomHudText('inform', 'Nothing nearby', 10000)
      else
          TriggerEvent('tp:civuncuff')
          TriggerEvent('removelockpick')
      end

  elseif (closestPdistance > closestVdistance) then
      if closestVdistance > 3.0 then
          exports['mythic_notify']:DoCustomHudText('inform', 'There is nobody nearby', 10000)
