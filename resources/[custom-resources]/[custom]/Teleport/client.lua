...
AddEventHandler('tp:hasEnteredMarker', function(zone)
	...
	if zone == 'HellCustomsEntry' then
        CurrentAction = zone
        CurrentActionMsg  = "Push ~INPUT_PICKUP~ to enter the shop."
    end

    if zone == 'HellCustomsExit' then
        CurrentAction = zone
        CurrentActionMsg  = "Push ~INPUT_PICKUP~ to exit the shop."
    end
