...
AddEventHandler('phone:addContact', function(name, number)
    ...
    MySQL.Async.execute('INSERT INTO phone_contacts (identifier, name, number) VALUES (@identifier, @name, @number)', {
        ...
        TriggerEvent('getotherContacts', true, src)
...
RegisterNetEvent('getotherContacts')
AddEventHandler('getotherContacts', function(player, pid)
    local src = pid
    local xPlayer = ESX.GetPlayerFromId(src)
    MySQL.Async.fetchAll('SELECT * FROM phone_contacts WHERE identifier = @identifier', { ['@identifier'] = xPlayer.getIdentifier() }, function(contacts)
        TriggerClientEvent('phone:loadContacts', src, contacts)
...
AddEventHandler('phone:callContact', function(targetnumber, toggle)
    ...
    getSourceFromIdentifier(targetIdentifier, function (srcTo)
...
RegisterNetEvent('phone:callAnonContact')
AddEventHandler('phone:callAnonContact', function(targetnumber, toggle)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    local targetIdentifier = getIdentifierByPhoneNumber(targetnumber)
    local xPlayers = ESX.GetPlayers()
    ...
    TriggerClientEvent('phone:initiateAnonCall', src, src)
    ...
    TriggerClientEvent('phone:receiveAnonCall', playerID, targetnumber, src, srcPhone)
end)
...
--[[ Keys App ]]
local allHouses = {}
RegisterNetEvent('phone:getHousingData')
AddEventHandler('phone:getHousingData', function(allHousing)
    print("updated house list")
    allHouses = allHousing
end)

RegisterNetEvent('phone:getKeys')
AddEventHandler('phone:getKeys', function()
    local src = source
    local ownedkeys = {}
    local sharedkeys = {}
    ...
    MySQL.Async.fetchAll("SELECT * FROM housekeys WHERE identifier = @identifier", {
        ...
        myKeys = json.decode(result[1].keylist)
        for i=1, #myKeys do
            for k,v in pairs(allHouses) do
                local hasGarage = true
                if v.gheading == "{}" then hasGarage = false end
                if v.id == myKeys[i] then
                    if v.type == "house" then
                        if v.owner == identifier then
                            table.insert(ownedkeys, {coords = {x = v.doorx, y = v.doory, z = v.doorz}, houseid = v.id, house_model = v.prop, house_name = "House #"..v.id, type = v.type, garage = hasGarage})
                        else
                            table.insert(sharedkeys, {coords = {x = v.doorx, y = v.doory, z = v.doorz}, houseid = v.id, house_model = v.prop, house_name = "House #"..v.id, type = v.type, garage = hasGarage})
                        end
                    elseif v.type == "motel" then
                        if v.owner == identifier then
                            ...
                            -- print(sincelastpayment)
                            table.insert(ownedkeys, {coords = {x = v.doorx, y = v.doory, z = v.doorz}, houseid = v.id, house_model = v.prop, house_name = "Motel #"..v.id, type = v.type, garage = hasGarage, amountdue = v.price, days = sincelastpayment})
                        else
                            table.insert(sharedkeys, {coords = {x = v.doorx, y = v.doory, z = v.doorz}, houseid = v.id, house_model = v.prop, house_name = "Motel #"..v.id, type = v.type, garage = hasGarage})
                        end
                    elseif string.match(v.type, "apt") then
                        ...
                        local aptname = string.gsub(v.type, "apt ", "")
                        if v.owner == identifier then
                            table.insert(ownedkeys, {coords = {x = v.doorx, y = v.doory, z = v.doorz}, houseid = v.id, house_model = v.prop, house_name = aptname.." Apt #"..v.id, type = v.type, garage = hasGarage, amountdue = v.price, days = sincelastpayment})
                        else
                            table.insert(sharedkeys, {coords = {x = v.doorx, y = v.doory, z = v.doorz}, houseid = v.id, house_model = v.prop, house_name = aptname.." Apt #"..v.id, type = v.type, garage = hasGarage})
                        end
                    end
                end
            end
        end
        TriggerClientEvent('returnPlayerKeys', src, ownedkeys, sharedkeys)
    end)
...
--         end
--     else
--         TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You did not enter a valid number', length = 7000})
--     end
...
ESX.RegisterUsableItem('simcard', function(source)  
	local src = source
	TriggerClientEvent('drp-phone:numberchange', src)
end)

RegisterServerEvent('drp-phone:numberchange')
AddEventHandler('drp-phone:numberchange', function(newpnumber)
	local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
	MySQL.Async.fetchAll("SELECT phone_number FROM users WHERE phone_number = @phone_number", { 
        ['@phone_number'] = newpnumber
        }, function(result)
		if result[1] then
			TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text = 'Phone number already exists', length = 7000})
        else
			...
			MySQL.Async.fetchAll("SELECT phone_number FROM users WHERE identifier = @identifier", { 
				...
				if presult[1] and not presult[2] then
                    local oldnumber = presult[1]["phone_number"]
                    Wait(500)
                    TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text = 'Sim card changing (Do not open phone)', length = 7000})
                    UpdateContacts(oldnumber,newpnumber)
                    Wait(500)
                    UpdateTextsSender(oldnumber,newpnumber)
                    Wait(500)
                    UpdateTextsReceiver(oldnumber,newpnumber)
                    Wait(500)
                    UpdateNumber(oldnumber,newpnumber)
                    Wait(500)
                    TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'success', text = 'Sim card changed, we can now use our phone!', length = 7000})
                    local identifier = getPlayerID(src)
                    getOrGeneratePhoneNumber(src, identifier)
                    MySQL.Async.fetchAll('SELECT * FROM phone_contacts WHERE identifier = @identifier', { ['@identifier'] = xPlayer.getIdentifier() }, function(contacts)
                        TriggerClientEvent('phone:loadContacts', -1, contacts)
                    end)
				else
					TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text = 'Failed to get number or old number does not exist?', length = 7000})
				end
			end)
		end
    end)
...
function UpdateContacts(oldnumber,newnumber)
    if oldnumber and newnumber then
        MySQL.Async.execute("UPDATE phone_contacts SET number = @newnumber WHERE number = @oldnumber", { 
            ['@oldnumber'] = oldnumber,
            ['@newnumber'] = newnumber,
        })
    end
...
function UpdateTextsSender(oldnumber,newnumber)
    if oldnumber and newnumber then
        MySQL.Async.execute("UPDATE phone_messages SET sender = @newnumber WHERE sender = @oldnumber", { 
            ['@oldnumber'] = oldnumber,
            ['@newnumber'] = newnumber,
        })
    end
...
function UpdateTextsReceiver(oldnumber,newnumber)
    if oldnumber and newnumber then
        MySQL.Async.execute("UPDATE phone_messages SET receiver = @newnumber WHERE receiver = @oldnumber", { 
            ['@oldnumber'] = oldnumber,
            ['@newnumber'] = newnumber,
        })
    end
...
function UpdateNumber(oldnumber,newnumber)
    if oldnumber and newnumber then
        MySQL.Async.execute("UPDATE users SET phone_number = @newnumber WHERE phone_number = @oldnumber", { 
            ['@oldnumber'] = oldnumber,
            ['@newnumber'] = newnumber,
        })
    end
