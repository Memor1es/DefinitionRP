...
$(document).ready(function() {
    ...
    document.onkeyup = function(data) {
        ...
        if (data.which == 27 && gameOn) {
            console.log('closing game?')
            gameOn = false;
            $.post('http://phone/close', JSON.stringify({}));
            $.post('http://phone/gameClose', JSON.stringify({}));
        ...
        if (action !== undefined) {
            switch (action) {
                ...
                case "dabcoin":
                    ...
                    break;
                // case "btnSetHouseGps":
                    // houseid = $(this).data('action-data');
                    // console.log($(this).data('action-data'));
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.startGame === true) {
            gameOn = true;
            openPhoneShell();
            openContainer('game');
            setTimeout(function() {
                playGame(item.data.tries, item.data.failures, item.data.duration, item.data.letters, item.data.chars);
                polyfillKey();
            }, 2000);
        }

        if (item.endGame === true) {
            closePhoneShell();
            gameOn = false;
        }
        ...
        switch (item.openSection) {
            ...
            case "Garage":
                ...
                addVehicles(item.vehicleData, item.showCarPaymentsOwed, item.impoundedvehicles)
            ...
            case "manageKeys":
                ...
                addManageKeys(item.sharedKeys, item.houseid);
                openContainer('manage-keys')
                break;
            case "giveKeys":
                $('.give-keys-entries').empty();
                addGiveKeys(item.namelist, item.houseid);
                openContainer('give-keys')
                break;
            case "settings":
...
function playGame(tries, failures, duration, letters, lets=null) {
    var LETTERS 
    if(lets == null) {
        LETTERS = ['w','a','s','d','i','j','k','l'];
    } else {
        LETTERS = Object.values(lets);
    }
    var animations = {'a':[],'b':[],'c':[],'d':[],'e':[],'f':[],'g':[],'h':[],'i':[],'j':[],'k':[],'l':[],'m':[],'n':[],'o':[],'p':[],'q':[],'r':[],'s':[],'t':[],'u':[],'v':[],'w':[],'x':[],'y':[],'z':[]};
    if(letters !== undefined && letters !== null) {
        timeOffset = letters
    } else {
        timeOffset = 2000; //interval between letters starting, will be faster over time
    }
    if(duration !== undefined && duration !== null) {
        DURATION = duration
    } else {
        DURATION = 5000; //interval between letters starting, will be faster over time
    }
    if(failures !== undefined && failures !== null) {
        maxFailures = failures
    } else {
        maxFailures = 10; //interval between letters starting, will be faster over time
    }
    if(tries !== undefined && tries !== null) {
        maxTries = tries
    } else {
        maxTries = 20; //interval between letters starting, will be faster over time
    }
    ...
    var score = 0
    var rate = 1.2;
    var RATE_INTERVAL = .25; //playbackRate will increase by .05 for each letter... so after 20 letters, the rate of falling will be 2x what it was at the start
    var counter = 0;
    misses = 0;
    ...
    function gameOver() {
      ...
      if(misses < maxFailures) {
        if(gameOn === true) {  
            $.post('http://phone/gameResult', JSON.stringify({ result: true }));
        }
      } else {
        if(gameOn === true) {
            $.post('http://phone/gameResult', JSON.stringify({ result: false }));
        }
      }
      gameOn = false;
    }
        ...
        if (animations[char] && animations[char].length) {
          ...
          target.animate([
            ...
            if(counter >= maxTries) {
                ...
                if((Math.floor(pos.top)) > 430 && (Math.floor(pos.top)) < 470) {
...
function addManageKeys(keys, houseid) {
    /* for (let key in keys) {
    ...
    $('.manage-keys-house').text("Managing keys for House #" + houseid);
    $('.manage-keys-house').css({"text-align" : "center"});
    if (keys.length === 0){
        let giveHouseKey = `
            <li class="collection-item">
                <div class="row no-padding">
                    <div class="col s9">
                        <span  class="truncate" style="font-weight:bold">Noone else has keys</span>
                    </div>
                </div>
            </li>
        `
        $('.manage-keys-entries').append(giveHouseKey);
    }
    for (let key in keys) {
        let manageHouseKey = `
            <li class="collection-item">
                <div class="row no-padding">
                    <div class="col s9">
                        <span  class="truncate" style="font-weight:bold">${keys[key].name}</span>
                    </div>
                    <div class="col s3 right-align">
                        <span class="phone-button manage-keys-remove" data-house-id="${houseid}" data-player-id="${keys[key].identifier}" aria-label="Remove Key" data-balloon-pos="left"><i class="fas red-text fa-user-times"></i></span>
                    </div>
                </div>
            </li>
        `
...
function addGiveKeys(keys, houseid) {
    $('.give-keys-house').text("Managing keys for House #" + houseid);
    $('.give-keys-house').css({"text-align" : "center"});
    if (keys.length === 0){
        let giveHouseKey = `
            <li class="collection-item">
                <div class="row no-padding">
                    <div class="col s9">
                        <span  class="truncate" style="font-weight:bold">No Players Nearby</span>
                    </div>
                </div>
            </li>
        `
        $('.give-keys-entries').append(giveHouseKey);
    }
    for (let key in keys) {
        let giveHouseKey = `
            <li class="collection-item">
                <div class="row no-padding">
                    ...
                    <div class="col s3 right-align">
                        <span class="phone-button give-keys-give" data-house-id="${houseid}" data-player-id="${keys[key].identifier}" aria-label="Give Key" data-balloon-pos="left"><i class="fas green-text fa-user-plus"></i></span>
                    </div>
                </div>
            </li>
        `
        ...
        for (let i = 0; i < keys[keyType].length; i++) {
            ...
            <li data-key-type="${keyType}">
                <div class="collapsible-header">
                    ...
                    if(key.garage === true){
                        keyElement += `<i class="fas fa-car teal-text" data-house-type="" style="width:25%;" data-house-id="${key.houseid}"></i>`
                    }
                        keyElement += `<!-- <i class="fas fa-map-marker-alt teal-text gps-location-click" data-house-type="${key.house_model}" data-house-id="${key.houseid}" style="float:right"></i> -->
                    </div>
                </div>
            ...
            if (keyType === "ownedKeys") {
                ...
                if(key.type != "house"){
                    keyElement += `
                                            ...
                                            <li class="collection-item"><i class="fas fa-money-check-alt"></i> Your rent is $${key.amountdue}</li>
                                `
                }
            }
            keyElement += `
                                    <li class="collection-item"><i class="fas fa-map-marked-alt"></i>&nbsp;${key.street}</li>
                            </ul>
                        </div>
                    </div>
                    `
            if (keyType === "ownedKeys") {
                keyElement += `
                        <div class="row no-padding">
                            <div class="col s12 center-align no-padding button-row" >
                                ...
                                <button class="waves-effect waves-light btn-small set-house-gps" data-house-id="${key.houseid}" aria-label="Set GPS" data-balloon-pos="up"><i class="fas fa-map-marker-alt"></i></button>
                                <button class="waves-effect waves-light btn-small give-keys" data-house-id="${key.houseid}" aria-label="Give Key" data-balloon-pos="up"><i class="fas fa-key"></i></button>
                                <button class="waves-effect waves-light btn-small manage-keys" data-house-id="${key.houseid}" aria-label="Manage Keys" data-balloon-pos="up"><i class="fas fa-user-slash"></i></button>
                ...
                <div class="row no-padding">
                    <div class="col s12 center-align no-padding">
                        ...
                        <!-- <button class="waves-effect waves-light btn-small phone-button" data-action="btnMortgage" aria-label="Pay Mortgage" data-balloon-pos="up-right"><i class="fas fa-hand-holding-usd"></i></button> -->
                        <button class="waves-effect waves-light btn-small set-house-gps" data-house-id="${key.houseid}" aria-label="Set GPS" data-balloon-pos="up"><i class="fas fa-map-marker-alt"></i></button>
                        <button class="waves-effect waves-light btn-small remove-shared-key" data-house-id="${key.houseid}" data-house-model="${key.house_model}" aria-label="Drop key" data-balloon-pos="up"><i class="fas fa-user-slash"></i></button>
                    </div>
                </div>
                `
            }
...
function openRadio() {
    let browserRadio =
        `
        <object type="text/html" data="http://google.com" class="browser-radio-window">
        </object>
    `;
...
function addVehicles(vehicleData, showCarPayments, impoundedvehicles) {
    ...
    $('.garage-entries').append("<div class='impound-top'>Impound</div>")

    if (Object.keys(impoundedvehicles).length === 0)
    {
        $('.garage-entries').append("<div class='impound-bottom'>No vehicles in impound.</div>")
    } else
    {
        $('.garage-entries').append("<div class='impound-bottom'>The following vehicles are impounded</div>")
    }

    for (let vehicle of Object.keys(impoundedvehicles)) {
        // console.log("impound vehicle")
        let carIconColor = "green";
        let carBGColor = "";
        if (impoundedvehicles[vehicle].amountDue > 0)
            carIconColor = "red";
        else if (impoundedvehicles[vehicle].amountDue == 0 && impoundedvehicles[vehicle].payments > 0)
            carIconColor = "orange";
        else
            carIconColor = "green";

        impoundedvehicles[vehicle].garage = "IMPOUND";
        carBGColor = "red";
        ...
        var vehicleElement = `
            <li>
                <div class="collapsible-header">
                    <i class="fas fa-car ${carIconColor}-text"></i>${impoundedvehicles[vehicle].name}
                    <span class="new badge" style="text-overflow:ellipsis;overflow:hidden;max-width:15ch;white-space:nowrap;background-color:${carBGColor}" data-badge-caption="">${impoundedvehicles[vehicle].garage}</span>
                </div>
                <div class="collapsible-body garage-body">
                    <div class="row">
                        <div class="col s12"> 
                            <ul class="collection">
                                <li class="collection-item"><i class="fas fa-map-marker-alt"></i>  ${impoundedvehicles[vehicle].garage}</li>
                                <li class="collection-item"><i class="fas fa-closed-captioning"></i> ${impoundedvehicles[vehicle].plate}</li>
                            </ul>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col s12 center-align">
                            <button class="waves-effect waves-light btn-small garage-track" data-plate="${impoundedvehicles[vehicle].plate}"><i class="fas fa-map-marker-alt"></i> Track</button>
                        </div>
                    </div>
                </div>
            </li>
...
$('.keys-entries').on('click', '.manage-keys', function () { // Retrieves list of people with key to your house
    $.post('http://phone/btnManageKeys', JSON.stringify({
        house_id: $(this).data('house-id')
    }));
});

$('.keys-entries').on('click', '.give-keys', function () { // Retrieves list of people with key to your house
    // console.log("GIVE KEY");
    $.post('http://phone/btnGiveKeys', JSON.stringify({
        house_id: $(this).data('house-id')
    }));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) { // Dropping a key
...
$('.keys-entries').on('click', '.set-house-gps', function(e) { // Setting GPS to house
    $.post('http://phone/setHouseGPS', JSON.stringify({
        houseid: $(this).data('house-id'),
    }))
});

$('.manage-keys-entries').on('click', '.manage-keys-remove', function () { // Remove someone elses key to your house
    $.post('http://phone/removeHouseKey', JSON.stringify({
        identifier: $(this).data('player-id'),
        house_id: $(this).data('house-id')
    }))
    $(this).closest('li').remove()
})

$('.give-keys-entries').on('click', '.give-keys-give', function () { // Remove someone elses key to your house
    $.post('http://phone/giveHouseKey', JSON.stringify({
        identifier: $(this).data('player-id'),
        house_id: $(this).data('house-id')
    }))
    $(this).closest('li').remove()
