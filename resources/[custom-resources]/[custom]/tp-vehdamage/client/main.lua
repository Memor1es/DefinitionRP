        ...
        if currentVehicle ~= nil and currentVehicle ~= false and currentVehicle ~= 0 then
            ...
            if driverPed == GetPlayerPed(-1) then
                ...
                if frameBodyChange ~= 0 then
                    ...
                    if lastFrameVehiclespeed > 110 and thisFrameVehicleSpeed < (lastFrameVehiclespeed * 0.75) and not damagedone then -- Increase threshold for crash speed

                        -- print("CRASHED")

                        -- print(lastFrameVehiclespeed, thisFrameVehicleSpeed)

                        local harness = true

                        if frameBodyChange > 20.0 then
                            -- print("BIG OL CRASH")
                            ...
                            if not seatbelt and not IsThisModelABike(currentVehicle) then
                                -- print("seatbelt off")
                                if math.random(math.ceil(lastFrameVehiclespeed)) > 25 then
                                    ejectionLUL()                           
                                end
                            elseif seatbelt and not IsThisModelABike(currentVehicle) then
                                -- print("seatbelt on")
                                if lastFrameVehiclespeed > 150 then
                                    if math.random(math.ceil(lastFrameVehiclespeed)) > 99 then -- HERE
                                        TriggerServerEvent('drp:harnessCheck')
                                    end
                                end
                            end
                        else
...
RegisterNetEvent('drp:harnessEject')
AddEventHandler('drp:harnessEject', function()
    ejectionLUL()
end)
