...
window.addEventListener("message", function(event) {
    if (event.data.action == "display") {
        ...
        secondInventorySetup(event.data.invOwner, event.data.itemList, event.data.invTier, event.data.name);
...
function closeInventory() {
    ...
    $.post("https://disc-inventoryhud/NUIFocusOff", JSON.stringify({}));
}
...
function secondInventorySetup(invOwner, items, invTier, name=null) {
    if(secondTier.label == "Shop") {
        $('#count').val('1');
    } else {
        $('#count').val('0')
    }
...
function setupSecondarySlots(owner) {
    ...
    for (i = 1; i <= (secondTier.slots); i++) {
        ...
        if (owner.startsWith("drop") || owner.startsWith("container") || owner.startsWith("car") || owner.startsWith("pd-trash") || owner.startsWith("bench")) {
            $('#inventoryTwo').find('.slot-template').addClass('temporary');
        } else if (owner.startsWith("pv") || owner.startsWith("stash") || owner.startsWith("evidence") || owner.startsWith("housing")) {
...
$(document).ready(function() {
    ...
    $('#inventoryOne, #inventoryTwo').on('click', '.slot', function(e) {
        ...
        if (dragging) {
            ...
            if (itemData !== undefined) {
                ...
                $(draggingItem).data('invTier', $(this).parent().data('invTier'));
                ...
                if ($(this).parent().data('invTier').name == "craft") {
                    $("#use").addClass("disabled");
                    $("#give").addClass("disabled");
    ...
    $('#use').click(function(event, ui) {
        if (dragging) {
            ...
            if (itemData.usable) {
                ...
                $.post("https://disc-inventoryhud/UseItem", JSON.stringify({
    ...
    $('#give').click(function(event, ui) {
        if (draggingItem != null && dragging) {
            ...
            // let printData = draggingItem.data('invTier');
            /* $.post("https://disc-inventoryhud/print", JSON.stringify({
                item: printData,
            })); */

            if (itemData.canRemove && ($(draggingItem).data('invTier').name != "craft")) {
                $.post("https://disc-inventoryhud/GetNearPlayers", JSON.stringify({
                    originItem: itemData,
                    action: 'give'
                }));
    ...
    $("#seize").mouseenter(function() {
        ...
        $.post("https://disc-inventoryhud/SeizeCash", JSON.stringify({
    ...
    $("#steal").mouseenter(function() {
        ...
        $.post("https://disc-inventoryhud/StealCash", JSON.stringify({
            target: destinationOwner
        }));
        closeInventory()
    });
    ...
    $('#drop').click(function(event, ui) {
        if (dragging) {
            ...
            if (itemData.canRemove) {
                ...
                $.post("https://disc-inventoryhud/DropItem", JSON.stringify({
                    item: itemData,
                    qty: dropCount
                }));
                successAudio.play();
        ...
        if (itemData !== undefined) {
            ...
            if (itemData.recipe !== undefined) {
                ...
                if (itemData.description !== undefined) {
                    ...
                    // $('.tooltip-div').find('.tooltip-desc').hide()
                }
            }
            ...
            if (itemData.weight !== undefined) {
                // $('.tooltip-div').find('.tooltip-meta').html('Weight: ' + itemData.weight * itemData.qty + "kg");
                $('.tooltip-div').find('.tooltip-meta').hide()
            } else {
                $('.tooltip-div').find('.tooltip-meta').hide()
            }
...
function AttemptDropInEmptySlot(origin, destination, moveQty) {
    ...
    if (result === -1) {
        ...
        if (origin.parent().data('invTier').name === "player") {
            if (destination.parent().data('invTier').name === "craft") {
                return;
            }
        }

        if (origin.parent().data('invTier').name === "craft") {
            ...
            $.post("https://disc-inventoryhud/craft", JSON.stringify({
        ...
        if (moveQty === item.qty) {
            ...
            InventoryLog('Moving ' + item.qty + ' ' + item.label + ' ' + ' From [' + origin.parent().data('invTier').name + "] " + origin.data('invOwner') + ' Slot ' + origin.data('slot') + ' To [' + destination.parent().data('invTier').name + "] " + destination.parent().data('invOwner') + ' Slot ' + item.slot);
            $.post("https://disc-inventoryhud/MoveToEmpty", JSON.stringify({
                originOwner: origin.parent().data('invOwner'),
                originSlot: origin.data('slot'),
                originTier: origin.parent().data('invTier'),
                originItem: item,
            ...
            InventoryLog('Empty: Moving ' + moveQty + ' ' + item.label + ' From [' + origin.parent().data('invTier').name + "] " + origin.data('invOwner') + ' Slot ' + item.slot + ' To [' + destination.parent().data('invTier').name + "] " + destination.parent().data('invOwner') + ' Slot ' + item2.slot);

            $.post("https://disc-inventoryhud/EmptySplitStack", JSON.stringify({
                originOwner: origin.parent().data('invOwner'),
                originSlot: origin.data('slot'),
                originTier: origin.parent().data('invTier'),
                originItem: origin.find('.item').data('item'),
    ...
    if (result === -1) {
        ...
        if (originItem.itemId === destinationItem.itemId && destinationItem.stackable) {
            ...
            if (moveQty != originItem.qty && destinationItem.qty + moveQty <= destinationItem.max) {
                ...
                InventoryLog('Non-Empty: Moving ' + moveQty + ' ' + originItem.label + ' In [ ' + origin.parent().data('invTier').name + "] " + origin.data('invOwner') + ' Slot ' + originItem.slot + ' To [' + destination.parent().data('invTier').name + "] " + destination.parent().data('invOwner') + ' Slot' + destinationItem.slot);
                $.post("https://disc-inventoryhud/SplitStack", JSON.stringify({
                    originOwner: origin.parent().data('invOwner'),
                    originTier: origin.parent().data('invTier'),
                ...
                if (destinationItem.qty === destinationItem.max) {
                    ...
                    InventoryLog('Swapping ' + originItem.label + ' In  [' + destination.parent().data('invTier').name + "] " + destination.parent().data('invOwner') + ' Slot ' + originItem.slot + ' With ' + destinationItem.label + ' In [' + origin.parent().data('invTier').name + origin.data('invOwner') + ' Slot ' + destinationItem.slot);
                    $.post("https://disc-inventoryhud/SwapItems", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originItem: origin.find('.item').data('item'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                    ...
                    successAudio.play();
                    InventoryLog('Merging Stack Of ' + originItem.label + ' In [' + origin.parent().data('invTier').name + "] " + origin.data('invOwner') + ' Slot ' + originItem.slot + ' To [' + destination.parent().data('invTier').name + "] " + destination.parent().data('invOwner') + ' Slot' + destinationItem.slot + " | Stack size is " + destinationItem.qty);
                    $.post("https://disc-inventoryhud/CombineStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
                    ...
                    InventoryLog('Topping Off Stack ' + originItem.label + ' To Existing Stack In Inventory [' + destination.parent().data('invTier').name + "] " + destination.parent().data('invOwner') + ' Slot ' + destinationItem.slot + " | Stack size is " + destinationItem.qty + ".");
                    $.post("https://disc-inventoryhud/TopoffStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
            ...
            InventoryLog('Swapping ' + originItem.label + ' In [' + origin.parent().data('invTier').name + '] ' + origin.data('invOwner') + ' Slot ' + destinationItem.slot + ' With ' + destinationItem.label + ' In [' + destination.parent().data('invTier').name + "] " + destination.parent().data('invOwner') + ' Slot ' + originItem.slot);
...
function AddItemToSlot(slot, data) {
    ...
    if (data.price !== undefined && data.price !== 0) {
        slot.find('.item-count').css({"display":"none"});
    } else {
        slot.find('.item-count').css({"display":"block"});
...
function InventoryLog(log) {
    ...
    if (log === 'Closing') {
        ...
        $.post("https://disc-inventoryhud/InvLog", JSON.stringify({
            message: log
        }))
    }
    ...
    if (cooldown === false) {
        ...
        if (action === "nearPlayersGive") {
            ...
            if (givingItem != null) {
                ...
                $.post("https://disc-inventoryhud/GiveItem", JSON.stringify({
                    target: target,
            ...
            $.post("https://disc-inventoryhud/GiveCash", JSON.stringify({
                target: target,
                item: givingItem,
                count: count
            }), function(status) {
