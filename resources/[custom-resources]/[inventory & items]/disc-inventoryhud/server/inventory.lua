...
RegisterServerEvent('disc-inv:closeAllInv')
AddEventHandler('disc-inv:closeAllInv', function()
    local source = source
...
AddEventHandler(
	...
	function(data)
		...
		if data.originOwner == data.destinationOwner and data.originTier.name == data.destinationTier.name then
            ...
            originInvHandler.applyToInventory(data.originOwner,
            function(inventory)
...
AddEventHandler(
	...
	function(data)
		...
		--TriggerClientEvent('DRP:Inventory:ClientGiveAnimation', tonumber(data.target))
		...
		TriggerClientEvent('DRP:Inventory:ClientGiveAnimation', tonumber(source))
		TriggerClientEvent("disc-inventoryhud:refreshInventory", source)
		TriggerClientEvent("disc-inventoryhud:refreshInventory", data.target)
	end
)
...
function createDisplayItem(item, esxItem, slot, price, type, description, recipe)
	local max = 100
	return {
		id = esxItem.name,
		itemId = esxItem.name,
		...
		giveable = true,
        description = description or getItemDataProperty(esxItem.name, "description"),
        recipe = false or recipe,
...
function getDisplayInventory(identifier, type, cb, source)
	...
	InvType[type].getInventory(identifier, function(inventory)
			...
			for k, v in pairs(inventory) do
				...
				local item = createDisplayItem(v, esxItem, tonumber(k), nil, nil, nil, nil) -- (item, esxItem, slot, price, type, description, recipe)
				item.usable = false
				item.giveable = false
...
function refreshLocker(identifier, type)
...
RegisterServerEvent('disc-inv:refreshLocker')
AddEventHandler('disc-inv:refreshLocker', function(identifier, type)
    refreshLocker(identifier, type)
end)