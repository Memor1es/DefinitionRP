...
RegisterNetEvent('disc-inventoryhud:frisk')
AddEventHandler('disc-inventoryhud:frisk', function()
    local player = ESX.GetPlayerData()
    if player.job.name == 'police' then
        local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
        if closestPlayer ~= -1 and closestDistance <= 3.0 then
            local searchPlayerPed = GetPlayerPed(closestPlayer)
            ESX.TriggerServerCallback('disc-inventoryhud:getIdentifier', function(identifier)
                TriggerServerEvent('disc-inventoryhud:friskdown', identifier)
            end, GetPlayerServerId(closestPlayer))
        end
    end
end)
