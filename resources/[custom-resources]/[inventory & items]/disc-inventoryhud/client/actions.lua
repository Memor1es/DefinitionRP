...
Citizen.CreateThread(function()
    while true do
        ...
        for k, v in pairs(keys) do
            if IsDisabledControlJustReleased(0, v) and not exports['tp-framework']:getZiptieStatus() then
                UseItem(k)
            end
        end
        if IsDisabledControlJustReleased(0, 37) and not exports['tp-framework']:getZiptieStatus() then
