...
CarParts = {  --- 
    [1] = {["x"] = 2351.426, ["y"] = 3131.929, ["z"] = 48.20}, --- Door 1
    [2] = {["x"] = 2352.423, ["y"] = 3134.446, ["z"] = 48.20},  --- Door 2
    [3] = {["x"] = 2353.213, ["y"] = 3131.664, ["z"] = 48.20}, --- Door 3
    [4] = {["x"] = 2353.836, ["y"] = 3134.129, ["z"] = 48.20}, --- Door 4
    [5] = {["x"] = 2350.366, ["y"] = 3132.157, ["z"] = 48.20}, --- Hood
	[6] = {["x"] = 2353.992, ["y"] = 3131.275, ["z"] = 48.20}, --- Trunk
	[7] = {["x"] = 2351.470, ["y"] = 3131.923, ["z"] = 48.20}, --- Battery
}

--[[ 
    {x =2351.4267578125, y =3131.9299316406, z =48.208744049072} door 1
    {x =2353.2133789063, y =3131.6645507813, z =48.20874786377} door 2
    {x =2352.4238281250, y =3134.4467773438, z =48.20874786377} door 3
    {x =2353.8369140625, y =3134.1298828125, z =48.20874786377} door 4
    {x =2350.3664550781, y =3132.1577148438, z =48.208724975586} hood
    {x =2353.9926757813, y =3131.2751464844, z =48.2087059021} trunk
    {x =2351.4709472656, y =3131.923828125, z =48.208724975586} - battery

]]

ChopCarLocation = {  --- Coords [x] = Door Coords [x2] =location if need be
    [1] = {["x"] = 2352.163, ["y"] = 3133.123, ["z"] = 48.20}, 
    [2] = {["x"] = 2348.390, ["y"] = 3139.473, ["z"] = 48.20}, 
}
...
--[[ Carry
	...
	if not IsEntityPlayingAnim(PlayerPedId(), "anim@heists@narcotics@trash", "walk", 3) then
		...
		if not IsEntityAttachedToEntity(partID, PlayerPedId()) then
			...
			Draw3DText2(ChopCarLocation[2]["x"], ChopCarLocation[2]["y"], ChopCarLocation[2]["z"] - 0.0, tostring("~w~~g~[E]~w~ Deliver Part"))
...
local veh = GetClosestVehicle(ChopCarLocation[1]["x"], ChopCarLocation[1]["y"], ChopCarLocation[1]["z"], 4.001, 0, 70)
	...
	if chopdoor6 then
	 ...
	 if chopdoor8 then
		...
		door8gone = true
		door8delivered = true
			TriggerServerEvent('tp-scrapyard:success', 0, "battery-acid") -- 0 = no money
			Citizen.Wait(500)
			TriggerServerEvent('tp-scrapyard:success', 0, "empty-battery") -- 0 = no money
			Citizen.Wait(500)
			disablecontrols = false
		end)
	 end
...
Citizen.CreateThread(function ()
	while true do
		...
		if allowedchop and chopstarted and not door1gone and not door1delivered then 
				DrawMarker(27,CarParts[1]["x"], CarParts[1]["y"], CarParts[1]["z"] - 0.9, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255, 255, 255, 155, 0, 0, 2, 0, 0, 0, 0)
	  ...
	  if allowedchop and chopstarted and not door2gone and door1delivered and not door2delivered then 
				DrawMarker(27,CarParts[2]["x"], CarParts[2]["y"], CarParts[2]["z"] - 0.9, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255, 255, 255, 155, 0, 0, 2, 0, 0, 0, 0)
	  ...
	  if allowedchop and chopstarted and not door3gone and door2delivered and not door3delivered then 
				DrawMarker(27,CarParts[3]["x"], CarParts[3]["y"], CarParts[3]["z"] - 0.9, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255, 255, 255, 155, 0, 0, 2, 0, 0, 0, 0)
	  ...
	  if allowedchop and chopstarted and not door4gone and door3delivered and not door4delivered then 
				DrawMarker(27,CarParts[4]["x"], CarParts[4]["y"], CarParts[4]["z"] - 0.9, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255, 255, 255, 155, 0, 0, 2, 0, 0, 0, 0)
	  ...
	  if allowedchop and chopstarted and not door5gone and door4delivered and not door5delivered then 
				DrawMarker(27,CarParts[5]["x"], CarParts[5]["y"], CarParts[5]["z"] - 0.9, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255, 255, 255, 155, 0, 0, 2, 0, 0, 0, 0)
	  ...
	  if allowedchop and chopstarted and not door6gone and door5delivered and not door6delivered then 
				DrawMarker(27,CarParts[6]["x"], CarParts[6]["y"], CarParts[6]["z"] - 0.9, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 255, 255, 255, 155, 0, 0, 2, 0, 0, 0, 0)
