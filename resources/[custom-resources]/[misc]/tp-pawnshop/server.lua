...
Citizen.CreateThread(function()
    for i=1, Config.MaxShops, 1 do
        ...
        addActiveShop(Config.PawnShops[mth].x, Config.PawnShops[mth].y, Config.PawnShops[mth].z, Config.PawnShops[mth].h, Config.PawnShops[mth].info)
        Citizen.Wait(1)
    end
...
function addActiveShop(shopx, shopy, shopz, shoph, shopinfo)
    local doesExist = doesShopExist(shopx, shopy, shopz, shoph, shopinfo)
    if doesExist then
        Citizen.Wait(100)
        local mth = math.random(1,#Config.PawnShops)
        addActiveShop(Config.PawnShops[mth].x, Config.PawnShops[mth].y, Config.PawnShops[mth].z, Config.PawnShops[mth].h, Config.PawnShops[mth].info)
    else
        table.insert(activeShops, {x = shopx, y = shopy, z = shopz, h = shoph, info = shopinfo})
    end
end
...
function doesShopExist(shopx, shopy, shopz, shoph, shopinfo)
    print(shopx .. " " .. shopy .. " " .. shopz .. " " .. shoph)
    for k,v in pairs(activeShops) do
        if v.x == shopx and v.y == shopy and v.z == shopz and v.h == shoph and v.info == shopinfo then
