...
local smokingt = false
RegisterNetEvent('smokeweed:onPot')
AddEventHandler('smokeweed:onPot', function()

    if not smokingt then
        local randomLightBreak = math.random(100)
        if randomLightBreak > 90 then
            TriggerServerEvent('drp:weed:breaklighter')
            exports['mythic_notify']:SendAlert('error', 'Your lighter has broken', 7000)
        else
            smokingt = true
            TriggerServerEvent('useableitem:remove', "weed")
            ...
            Citizen.Wait(20000)
            smokingt = false
            exports['mythic_notify']:SendAlert('success', 'We need to smoke another', 7000)
            Citizen.Wait(100000) 
        ...
        exports['mythic_notify']:SendAlert('error', 'We already high bro', 7000)
    end
end)
