...
function MFF:Interact(closest)
  ...
  if closest.val == "LockpickDoor" then
    ...
    -- check police count
    ESX.TriggerServerCallback('tp:isRestartComing', function(isRestartComing)
        if isRestartComing then
            exports['mythic_notify']:SendAlert('error', 'Citywide Storm Alert is Still Active.', 12000)	
            SetTimeout(3000, function()
              self.Interacting = false
            end)
        else
            ESX.TriggerServerCallback('tprp:GetJobCountCallBack', function(jobCount)
                if jobCount >= MFF.MinPoliceCount then
  ...
  elseif closest.val == "OpenVault" then
    ESX.TriggerServerCallback('tp:isRestartComing', function(isRestartComing)
        if isRestartComing then
            ...
            ESX.TriggerServerCallback('tprp:GetJobCountCallBack', function(jobCount)
                if jobCount >= MFF.MinPoliceCount then
                  ESX.TriggerServerCallback('drp-framework:checkFleecaCooldown', function(cb)
                    if cb == true then
                      TriggerServerEvent('MF_Fleeca:NotifyPolice', closest)
                      self:HandleVaultDoor(closest)
                    else
                      self.Interacting = false
                      exports['mythic_notify']:SendAlert('error', 'This bank is on lockdown.', 7000)	
                    end
                  end)
        ...
        if hasPhone then
          ...
          TriggerEvent('DRP:Games:StartPhoneLetters', {
            ...
            time = 1200,
                ...
                Wait(100)
              if success then
                TriggerServerEvent('DRP:Fleeca:Server:RemoveItem', 'bankidcard', 1)
                exports['progressBars']:startUI(2 * 1000, "Requesting Access")
                Wait(2 * 1000)
                ...
                self.SafeOpen = true
                  ...
                  TriggerServerEvent('drp-framework:updateFleecaCooldownRemove')
                  exports['mythic_notify']:SendAlert('error', 'You failed entering the requested combination.')
              end
                ClearPedTasksImmediately(plyPed)
                self.Interacting = false
          end)
          ...
          self.Interacting = false
          exports['mythic_notify']:SendAlert('error', 'You require a phone to hack this bank vault.')    
        end
    else
      exports['mythic_notify']:SendAlert('error', 'You havn\'t got a bank ID card.')
      self.Interacting = false
    end
  end)
end
