...
function MFF:RewardPlayer(data,id)
  ...
  if id then
    ...
    for k,v in pairs(self.LootTable) do
      ...
      if amount > 0 then
        local matty = math.random(1500, 4800)
        -- xPlayer.addMoney(matty) 
        xPlayer.addAccountMoney('black_money', matty)
...
RegisterServerEvent('DRP:Fleeca:Server:RemoveItem')
AddEventHandler('DRP:Fleeca:Server:RemoveItem', function(item, number)
  local xPlayer = ESX.GetPlayerFromId(source)
  while not xPlayer do Citizen.Wait(0); xPlayer = ESX.GetPlayerFromId(source); end
  local count = xPlayer.getInventoryItem(item)
  if count then count = count.count; end
  if count and tonumber(count) >= tonumber(number) then 
    xPlayer.removeInventoryItem(item, tonumber(number))
  end
end)
