            ...
            if not safe then              
              ...
              if (not self.Looted or (self.Looted and not self.Looted[key])) and lootRemains then
                ...
                if Utils:GetKeyPressed("E") then
                    if not restartComing then
                        self:Interact(key,val, plyPed,false)
                    else
                        exports['mythic_notify']:SendAlert('error', 'Citywide Storm Alert is Still Active.', 7000)
                    end
                end
              end
...
function MFV:Interact(key,val, plyPed, safe)
  if not safe then
    ...
    if not matching and plyWeapon then
        ...
        exports["t0sic_loadingbar"]:StartDelayedFunction("Searching Cabinet...", 12000, function()
...
function MFV:SpawnGuardNPC()
  ...
  for k,v in pairs(nearby) do
    Citizen.CreateThread(function()
      ...
      TaskShootAtEntity(newPed, GetPlayerPed(v), 5000, GetHashKey("FIRING_PATTERN_FULL_AUTO"))
    end)
  end
end
