...
Citizen.CreateThread(function()
    while true do
        ...
        for k, v in pairs(items) do
            nearest = GetClosestObjectOfType(playerCoords.x, playerCoords.y, playerCoords.z, 5.0, GetHashKey(v), false, false, false)
            if nearest ~= 0 then
                break
            end
        end
        ...
        if nearest ~= 0 and distance < 5 then
...
function processScrap(nearest, nearestCoords)
    ...
    exports["t0sic_loadingbar"]:StartDelayedFunction("Removing parts...", 15000, function()
        ...
        exports['mythic_notify']:SendAlert('success', 'You removed some parts', 10000)
...
AddEventHandler('tp-scrap:notifyCopsCL', function(coords)
    ...
    if ESX.PlayerData.job.name == "police" then
        exports['mythic_notify']:SendAlert('error', 'Someone was spotted damaging property', 15000)
        Citizen.CreateThread(function()
