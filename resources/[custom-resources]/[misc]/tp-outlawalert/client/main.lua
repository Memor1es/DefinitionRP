...
Citizen.CreateThread(function()
	while true do
		...
		elseif IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and Config.GunshotAlert then
			...
			if (GetCurrentPedWeapon(playerPed) == GetHashKey('WEAPON_PISTOL50')) or (GetCurrentPedWeapon(playerPed) == GetHashKey('WEAPON_STUNGUN')) then
				break
			end

			Citizen.Wait(3000)

			if (alertForWeapon(weaponHash)) then
