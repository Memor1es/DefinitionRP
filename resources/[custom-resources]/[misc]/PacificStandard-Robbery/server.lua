...
ESX.RegisterUsableItem('thermite',function(source)
    ...
    cops = exports['tp-framework']:GetJobCount("police")
    restart = exports('isRestartComing', IsRestartComing)
    if restart then
        TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = 'Citywide Storm Alert is Still Active.', length = 12000})
    else
        if cops >= MFP.MinPoliceCount then
